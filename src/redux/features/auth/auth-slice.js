...
const updateProfile = createAsyncThunk('auth/updateProfile',
    async ({token, resetForm, setSubmitting, ...rest}, {rejectWithValue}) => {
        try {
            ...
            resetForm();
            setSubmitting(false);
            localStorage.setItem(CONSTANTS.GAB_AUTH_TOKEN, JSON.stringify(response.data.token));
            localStorage.setItem(CONSTANTS.GAB_AUTH_DATA, JSON.stringify(response.data.data));
